// MEFAI Social-Smart Divergence Â·Â·Â· ORIGINAL SKILL
// Problem: Retail follows social hype, smart money acts independently.
// When hype is HIGH but SM is SELLING Â·†’ distribution (trap for retail).
// When hype is LOW but SM is BUYING Â·†’ accumulation (smart money loading before pump).
...
export class SocialDivergencePanel extends BasePanel {
  ...
  async fetchData() {
    ...
    // Parse SM signals Â·Â·Â· aggregate by symbol
    const smMap = new Map();
    ...
    for (const sym of allSymbols) {
      ...
      results.push({
        symbol: sym,
        hypeScore,
        sentiment: social?.sentiment || 'Â·Â·Â·',
        smDir,
        smCount,
        divType,
  ...
  renderContent(data) {
    ...
    const divColors = {
      ...
      'CONFIRMÂ·†‘': '#f0b90b', 'CONFIRMÂ·†Â·': '#a855f7',
    };
    const divLabels = {
      ACCUM: 'ðŸŸ¢ ACCUMULATION', DISTRIB: 'ðŸÂ·´ DISTRIBUTION',
      'CONFIRMÂ·†‘': 'ðŸŸ¡ ALIGNED BULL', 'CONFIRMÂ·†Â·': 'ðŸŸ£ ALIGNED BEAR',
    };
    ...
    for (const r of sorted) {
      ...
      h += `<td class="${smCls}" style="font-weight:700">${r.smDir === 'buy' ? 'BUYÂ·†‘' : r.smDir === 'sell' ? 'SELLÂ·†Â·' : 'Â·Â·Â·'} (${r.smCount})</td>`;
      h += `<td class="val-num">${r.price ? '$' + formatPrice(r.price) : 'Â·Â·Â·'}</td>`;
      h += `<td class="${cls}">${r.change ? (r.change >= 0 ? 'Â·†‘' : 'Â·†Â·') + Math.abs(r.change).toFixed(2) + '%' : 'Â·Â·Â·'}</td>`;
      h += '</tr>';
    }
    h += '</tbody></table>';
    return h;
