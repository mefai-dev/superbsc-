// MEFAI BSC Chain Growth Tracker Â·Â·Â· TVL history + multi-chain comparison
...
export class ChainGrowthPanel extends BasePanel {
  ...
  _renderOverview(data) {
    ...
    h += `<div class="cg-card"><div class="cg-label">BSC TVL</div><div class="cg-val">${formatCurrency(data.currentTvl)}</div><div class="cg-sub ${c1d}">${chg1d >= 0 ? 'Â·†‘' : 'Â·†Â·'}${Math.abs(chg1d).toFixed(2)}% 24h</div></div>`;
    ...
    h += `<div class="cg-card"><div class="cg-label">7d Change</div><div class="cg-val ${c7d}">${chg7d >= 0 ? 'Â·†‘' : 'Â·†Â·'}${Math.abs(chg7d).toFixed(2)}%</div></div>`;
    h += `<div class="cg-card"><div class="cg-label">30d Change</div><div class="cg-val ${c30d}">${chg30d >= 0 ? 'Â·†‘' : 'Â·†Â·'}${Math.abs(chg30d).toFixed(2)}%</div></div>`;
    h += '</div>';
  ...
  _renderRanking(data) {
    ...
    data.topChains.forEach((c, i) => {
      ...
      h += `<td style="font-size:9px;color:var(--text-muted)">${escapeHtml(c.tokenSymbol || 'Â·Â·Â·')}</td>`;
      h += '</tr>';
    });
    h += '</tbody></table>';
    return h;
