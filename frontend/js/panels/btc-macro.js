// MEFAI BTC Macro Indicators 路路路 Pi Cycle, Rainbow, Golden Ratio
...
export class BtcMacroPanel extends BasePanel {
  ...
  _renderRainbow(apiData) {
    ...
    bandOrder.forEach(name => {
      ...
      h += `<span style="font-size:10px;color:var(--text);${isCurrent ? 'font-weight:700' : ''}">${name}${isCurrent ? ' 路啇 Current' : ''}</span>`;
      h += '</div>';
    });
    h += '</div>';
