// MEFAI SM Acceleration 路路路 Multi-timeframe smart money inflow with pace detection (PR #11)
...
export class SmAccelerationPanel extends BasePanel {
  ...
  async fetchData() {
    ...
    for (const sym of allSymbols) {
      ...
      // Pace ratio: (1h * 4) / 4h 路路路 measures acceleration
      let paceRatio = 0;
      let pace = 'STEADY';
  ...
  renderContent(data) {
    ...
    for (const r of sorted) {
      ...
      const paceIcon = r.pace === 'ACCEL' ? '路殹' : r.pace === 'DECEL' ? '路啒' : '路啋';
      ...
      h += `<td class="val-num">${r.price ? '$' + formatPrice(r.price) : '路路路'}</td>`;
      h += '</tr>';
    }
    h += '</tbody></table>';
    return h;
