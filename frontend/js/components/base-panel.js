// MEFAI Base Panel 路路路 Web Component base class for all panels
...
export class BasePanel extends HTMLElement {
    ...
    this.innerHTML = `
      <div class="panel-header">
        ...
        <div class="panel-actions">
          <button class="panel-refresh" title="Refresh">路喕</button>
        </div>
      </div>
  ...
  _skeletonHTML() {
    const msgs = window.mefaiI18n?.loadingMessages?.() || [
      'AI is analyzing markets', 'Scanning blockchain data',
      'Processing smart money signals', 'Fetching live market data',
      'Connecting to data feeds', 'AI models warming up', 'Syncing with exchange',
    ];
  ...
  async refresh() {
    ...
    try {
      ...
      if (this._data && !this._firstLoad) {
        // Silent fail 路路路 keep existing content
      } else {
