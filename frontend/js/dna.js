...
function merge(sm, fund, reg, bas, acc) {
  ...
  mv('v-comp', syms.length?(cS/syms.length).toFixed(1):'路路路', '');
  mv('v-rsi', syms.length?(rsiS/syms.length).toFixed(1):'路路路', '');
  mv('v-macd', syms.length?(macdS/syms.length).toFixed(3):'路路路', macdS>=0?'up':'dn');
  mv('v-vol', syms.length? (syms.reduce((a,s)=>a+Math.abs(m[s].volc),0)/syms.length).toFixed(1)+'%' :'路路路', '');
...
// 路晲路晲路晲 DATA FLY PARTICLES 路晲路晲路晲
let flyTimer;
function startDataFly() {
  if (flyTimer) clearInterval(flyTimer);
  flyTimer = setInterval(spawnFly, 500);
}
// Project 3D node position 路啋 2D screen coords relative to canvas-wrap
...
function updateFeeds() {
  $('feed-left').innerHTML = D.map(d => {
    ...
    return `<div class="fr" data-s="${d.s}"><span class="fr-s">${d.s.replace('USDT','')}</span><span class="fr-v" style="color:var(${c})">${d.comp.toFixed(1)}</span><div class="bar"><div class="bar-f" style="width:${d.comp}%;background:var(${c})"></div></div><span class="fr-v ${d.bias==='BULLISH'?'up':'dn'}">${d.bias==='BULLISH'?'路柌':'路柤'}</span></div>`;
  }).join('');
...
function build() {
  ...
  for (let i=0;i<n;i++) {
    ...
    // 路路路路路路 BRANCHES: RSI, MACD, OI, Funding 路路路路路路
    if (d.sig >= 0.25) {
      // RSI branch
