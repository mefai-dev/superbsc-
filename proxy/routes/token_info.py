...
# Common DEX token addresses 路啋 Binance CEX symbol mappings for kline fallback
_ADDR_TO_SYMBOL = {
...
# chainId 路啋 DQuery platform mapping
_CHAIN_TO_PLATFORM = {
    "56": "bsc",
    "bsc": "bsc",
    "1": "eth",
...
async def token_kline(
    ...
    # DQuery supported intervals 路路路 others need Binance spot fallback
    dquery_intervals = {
        "1m",
        "1h",
        "2h",
    ...
    if isinstance(result, dict):
        ...
        if status.get("error_code") not in (None, "0", 0):
            # DQuery failed 路路路 try Binance spot as last resort
            if symbol:
                fallback_url = f"{SPOT_BASE}/api/v3/klines"
                fallback_params = {
                    "symbol": symbol,
